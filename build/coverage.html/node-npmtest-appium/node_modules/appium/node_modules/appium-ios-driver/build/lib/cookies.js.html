...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-appium/node_modules/appium/node_modules/appium-ios-driver/build/lib/cookies.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-appium/node_modules/appium/node_modules/appium-ios-driver/build/lib/cookies.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../../../../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-appium/node_modules/appium/node_modules/appium-ios-driver/build/lib/</a> &#187; cookies.js</div>
</div>
<div class="body">
...
<span class="fstat-no" title="function not covered" >function convertCookie(value, converter) {</span>
...
<span class="cstat-no" title="statement not covered" >    value = value.slice(1, -1).replace(/\\"/g, '"').replace(/\\\\/g, '\\');</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  var parsedValue = undefined;</span>
<span class="cstat-no" title="statement not covered" >  try {</span>
<span class="cstat-no" title="statement not covered" >    parsedValue = decodeURIComponent(value.replace(/\+/g, ' '));</span>
  } catch (e) {
    // no need to fail if we can't decode
<span class="cstat-no" title="statement not covered" >    log.warn(e);</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  return converter ? converter(parsedValue) : parsedValue;</span>
}
&nbsp;
// takes arguments given and creates a JavaScript Cookie
<span class="fstat-no" title="function not covered" >function createJSCookie(key, value) {</span>
<span class="cstat-no" title="statement not covered" >  var options = arguments.length &lt;= 2 || arguments[2] === undefined ? {} : arguments[2];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  return [encodeURIComponent(key), '=', value, options.expires ? '; expires=' + options.expires : '', options.path ? '; path=' + options.path : '', options.domain ? '; domain=' + options.domain : '', options.secure ? '; secure' : ''].join('');</span>
}
&nbsp;
// takes the JavaScript cookieString and translates it into a JSONWire formatted cookie
<span class="fstat-no" title="function not covered" >function createJWPCookie(key, cookieString) {</span>
<span class="cstat-no" title="statement not covered" >  var converter = arguments.length &lt;= 2 || arguments[2] === undefined ? null : arguments[2];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  var result = {};</span>
<span class="cstat-no" title="statement not covered" >  var cookies = cookieString ? cookieString.split('; ') : [];</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >  try {</span>
<span class="cstat-no" title="statement not covered" >    for (var _iterator = _getIterator(cookies), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {</span>
<span class="cstat-no" title="statement not covered" >      var cookie = _step.value;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      var parts = cookie.split('=');</span>
&nbsp;
      // get the first and second element as name and value
<span class="cstat-no" title="statement not covered" >      var _name = decodeURIComponent(parts.shift());</span>
<span class="cstat-no" title="statement not covered" >      var val = parts[0];</span>
&nbsp;
      // if name is key, this is the central element of the cookie, so add as `name`
      // otherwise it is an optional element
<span class="cstat-no" title="statement not covered" >      if (key &amp;&amp; key === _name) {</span>
<span class="cstat-no" title="statement not covered" >        result.name = key;</span>
<span class="cstat-no" title="statement not covered" >        result.value = convertCookie(val, converter);</span>
      } else {
<span class="cstat-no" title="statement not covered" >        result[_name] = convertCookie(val, converter);</span>
      }
    }
  } catch (err) {
...
<span class="fstat-no" title="function not covered" >function getValue(key, cookieString) {</span>
<span class="cstat-no" title="statement not covered" >  var converter = arguments.length &lt;= 2 || arguments[2] === undefined ? null : arguments[2];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  var result = createJWPCookie(key, cookieString, converter);</span>
&nbsp;
  // if `key` is undefined we want the entire cookie
<span class="cstat-no" title="statement not covered" >  return _lodash2['default'].isUndefined(key) ? result : result.value;</span>
}
&nbsp;
// returns a cookie that expires on 01 Jan 1970
// assign the returned cookie to an existing cookie to delete that cookie
<span class="fstat-no" title="function not covered" >function expireCookie(key, options) {</span>
  // override `expires` in `options`, and then make the cookie
<span class="cstat-no" title="statement not covered" >  return createJSCookie(key, '', _lodash2['default'].assign({}, options, {</span>
