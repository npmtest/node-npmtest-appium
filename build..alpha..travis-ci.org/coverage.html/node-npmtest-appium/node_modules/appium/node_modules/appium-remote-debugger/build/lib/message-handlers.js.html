...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-appium/node_modules/appium/node_modules/appium-remote-debugger/build/lib/message-handlers.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-appium/node_modules/appium/node_modules/appium-remote-debugger/build/lib/message-handlers.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../../../../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-appium/node_modules/appium/node_modules/appium-remote-debugger/build/lib/</a> &#187; message-handlers.js</div>
</div>
<div class="body">
 ...
 */
&nbsp;
<span class="fstat-no" title="function not covered" >function onPageChange(appIdKey, pageDict) {</span>
  // save the page dict for this app
<span class="cstat-no" title="statement not covered" >  if (this.appDict[appIdKey]) {</span>
<span class="cstat-no" title="statement not covered" >    if (this.appDict[appIdKey].pageDict &amp;&amp; this.appDict[appIdKey].pageDict.resolve) {</span>
      // pageDict is a promise, so resolve
<span class="cstat-no" title="statement not covered" >      this.appDict[appIdKey].pageDict.resolve(pageDict);</span>
    }
...
<span class="cstat-no" title="statement not covered" >    _logger2['default'].debug('Page changed: ' + (0, _helpers.simpleStringify)(pageDict));</span>
<span class="cstat-no" title="statement not covered" >    this.emit(_remoteDebugger.RemoteDebugger.EVENT_PAGE_CHANGE, {</span>
      appIdKey: appIdKey.replace('PID:', ''),
      pageArray: (0, _helpers.pageArrayFromDict)(pageDict)
    });
  } else {
<span class="cstat-no" title="statement not covered" >    _logger2['default'].debug('Received page change notice for app \'' + appIdKey + '\' ' + ('but listening for \'' + this.appIdKey + '\'. Ignoring.'));</span>
  }
}
&nbsp;
<span class="fstat-no" title="function not covered" >function onAppConnect(dict) {</span>
<span class="cstat-no" title="statement not covered" >  var appIdKey = dict.WIRApplicationIdentifierKey;</span>
<span class="cstat-no" title="statement not covered" >  _logger2['default'].debug('Notified that new application \'' + appIdKey + '\' has connected');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  this.updateAppsWithDict(dict);</span>
}
&nbsp;
<span class="fstat-no" title="function not covered" >function onAppDisconnect(dict) {</span>
<span class="cstat-no" title="statement not covered" >  var appIdKey;</span>
<span class="cstat-no" title="statement not covered" >  return _regeneratorRuntime.async(<span class="fstat-no" title="function not covered" >function onAppDisconnect$(context$1$0) {</span></span>
<span class="cstat-no" title="statement not covered" >    while (1) <span class="cstat-no" title="statement not covered" >switch (context$1$0.prev = context$1$0.next) {</span></span>
      case 0:
<span class="cstat-no" title="statement not covered" >        appIdKey = dict.WIRApplicationIdentifierKey;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        _logger2['default'].debug('Application \'' + appIdKey + '\' disconnected. Removing from app dictionary.');</span>
<span class="cstat-no" title="statement not covered" >        _logger2['default'].debug('Current app is ' + this.appIdKey);</span>
&nbsp;
        // get rid of the entry in our app dictionary,
        // since it is no longer available
<span class="cstat-no" title="statement not covered" >        delete this.appDict[appIdKey];</span>
&nbsp;
        // if the disconnected app is the one we are connected to, try to find another
<span class="cstat-no" title="statement not covered" >        if (this.appIdKey === appIdKey) {</span>
<span class="cstat-no" title="statement not covered" >          _logger2['default'].debug('No longer have app id. Attempting to find new one.');</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (!this.appDict) {</span>
          // this means we no longer have any apps. what the what?
<span class="cstat-no" title="statement not covered" >          _logger2['default'].debug('Main app disconnected. Disconnecting altogether.');</span>
<span class="cstat-no" title="statement not covered" >          this.connected = false;</span>
<span class="cstat-no" title="statement not covered" >          this.emit(_remoteDebugger.RemoteDebugger.EVENT_DISCONNECT, true);</span>
        }
&nbsp;
      case 6:
      case 'end':
...
&nbsp;
<span class="fstat-no" title="function not covered" >function onAppUpdate(dict) {</span>
<span class="cstat-no" title="statement not covered" >  var appIdKey = dict.WIRApplicationIdentifierKey;</span>
<span class="cstat-no" title="statement not covered" >  _logger2['default'].debug('Notified that application \'' + appIdKey + '\' has been updated.');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  this.updateAppsWithDict(dict);</span>
}
&nbsp;
<span class="fstat-no" title="function not covered" >function onReportDriverList(dict) {</span>
<span class="cstat-no" title="statement not covered" >  _logger2['default'].debug('Notified of connected drivers: ' + JSON.stringify(dict.WIRDriverDictionaryKey) + '.');</span>
}
&nbsp;
var messageHandlers = {
